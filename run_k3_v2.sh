...
#
# The patch is 3 lines, bind-mounted over the image (see /opt/dlami/nvme/patch):
...
DP="${DP:-8}"          # DP attention degree; 1 disables it
# WHY DP ATTENTION IS REQUIRED FOR A REALISTIC CHUNK:
#   validate_deepep_v2_dispatch_token_budget (moe_hook.py:381) enforces
#   chunked_prefill_size / dp_size <= CAP, and the v2 ElasticBuffer costs
#   ~10.5 GiB per 1024 of CAP (CAP=4096 asked for 42.00 GiB -> OOM at 24 GiB
#   free). Without DP attention dp_size=1, so an 8K chunk would need CAP=8192
#   (~84 GiB) -- impossible. With --dp-size 8 the same 8K chunk needs only
#   CAP=1024. This is also the shape deepep_v2 is actually deployed in.
DPARGS=()
[ "$DP" -gt 1 ] && DPARGS=(--enable-dp-attention --dp-size "$DP")
# Under DP attention the attention + mamba/linear-attention state caches are NO
# LONGER sharded by TP -- each DP rank holds its own -- so a MEMFRAC that worked
# at DP=1 leaves nothing for the ElasticBuffer (observed: 265.87 GiB of 267.68
# already allocated, OOM on a 588 MiB alloc). MAXRUN caps the number of hybrid
# states that must fit, which is the lever that actually frees memory here;
# MEMFRAC alone hits "Not enough GPU memory for hybrid state cache" instead.
...
# MEASURED allocation order and cost per B300 (267.68 GiB), MEMFRAC=0.84:
#   Load weight end   -> mem usage 214.88 GB, avail 51.11 GB
#   KV Cache          -> 6.73 GB / 261120 tokens, avail 42.54 GB
#   CUDA graph capture-> begins at avail 41.90 GB and eats >21 GB
#   DeepEP v2 buffer  -> LAST, and this is what OOMs
# So the competition is ElasticBuffer vs CUDA graph, not vs the KV pool. For a
# prefill measurement (OSL=1) the decode graphs are dead weight, so dropping them
# is what buys a realistic CHUNK. Keep them for the decode arm.
CGARGS=()
[ "${DISCG:-0}" = 1 ] && CGARGS=(--disable-cuda-graph)
...
# `docker rm -f` returns BEFORE the 8 scheduler processes have released their GPU
# memory. Relaunching immediately OOMs during *weight load* with a tiny alloc
# (seen: "Tried to allocate 588.00 MiB ... free: 18022400") which looks exactly
# like a mem-fraction problem and is not. Wait for the GPUs to actually drain.
